...
const BigFloat = @import("src/root.zig").BigFloat;
const utils = @import("src/test_utils.zig");
...
fn Context(BF: type, comptime op: TestOp) type {
    return struct {
        ...
        fn applyF(args: [arg_count]F) F {
            return switch (op) {
                .inv => 1.0 / args[0],
                .exp2 => @import("src/exp2.zig").exp2(args[0]),
                .log2 => @import("src/log2.zig").log2(args[0]),
                .add => args[0] + args[1],
                .sub => args[0] - args[1],
                .mul => args[0] * args[1],
                .div => args[0] / args[1],
...
test "fuzz inv" {
    const Ctx = Context(utils.EmulatedFloat(f32), .inv);
