...
test "pow" {
    ...
    inline for (utils.bigFloatTypes(&.{ f64, f80, f128 }, &.{ i31, i64 })) |F| {
        ...
        try utils.expectApproxEqRel(
            ...
            F.init(23.4).pow(.init(123456789)),
        ...
        try utils.expectApproxEqRel(
            ...
            F.init(1.000_000_000_1).pow(.init(123456789.01234)),
            large_power_tolerance,
        );
        try utils.expectApproxEqRel(
        ...
        if (math.floatFractionalBits(@FieldType(F, "significand")) + 1 >= @typeInfo(@FieldType(F, "exponent")).int.bits) {
            try testing.expectEqual(
                ...
                F.init(2).pow(.init(max_exp)),
            );
            try testing.expectEqual(
                F{ .significand = 1, .exponent = min_exp + 1 },
                F.init(2).pow(.init(min_exp + 1)),
            );
        }
        ...
        try testing.expectEqual(
            F.inf,
            F.max_value.neg().pow(.init(2)),
        );
        try utils.expectBitwiseEqual(
            F.init(-0.0),
            F.max_value.neg().pow(.init(-212389)),
        );
        try testing.expectEqual(
            F.init(0),
            F.min_value.pow(.init(2)),
        );
        try testing.expectEqual(
            F.min_value,
            F.min_value.pow(.init(1)),
        );
        try testing.expectEqual(
            F.inf,
            F.min_value.pow(.init(-1)),
        ...
        try testing.expectEqual(
            F.inf,
            F.init(-9.2553794e-23).pow(.init(-2.1465478e108)),
        );
    }
}
...
test "powi" {
    ...
    inline for (utils.bigFloatTypes(&.{ f64, f80, f128 }, &.{ i31, i64 })) |F| {
        ...
        try testing.expectEqual(
            F.max_value.neg().inv(),
            F.max_value.neg().powi(-1),
        );
        try testing.expectEqual(
            F{ .significand = 1, .exponent = max_exp },
            F.init(2).powi(max_exp),
        );
        try testing.expectEqual(
            F{ .significand = 1, .exponent = min_exp },
            F.init(2).powi(min_exp),
        );
        try testing.expectEqual(
            F.min_value,
            F.min_value.powi(1),
